...
def main() -> int:
    ...
    def _maybe_vote(ctx) -> None:
        ...
        try:
            ...
            if _is_session_closed(err):
                logger.info("growth vote skipped — session already closed proposalId=%s err=%s", pending["proposal_id"], err)
            else:
                raise
        ...
        emit_signal(
            ctx.actions,
            "session.ended",
            {"vote": vote, "recommendation": pending["recommendation"], "participantId": "growth-agent"},
        )
        participant.stop()
