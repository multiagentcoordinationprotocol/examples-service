...
export class LangChainHostAdapter implements AgentHostAdapter {
  readonly framework: AgentFramework = 'langchain';

  validateManifest(manifest: AgentManifest): ManifestValidationResult {
    const errors: string[] = [];

    if (manifest.framework !== 'langchain') {
      errors.push(`expected framework "langchain", got "${manifest.framework}"`);
    }
    ...
    if (entrypointType && entrypointType !== 'python_module' && entrypointType !== 'python_file') {
      errors.push(`langchain entrypoint type must be python_module or python_file, got "${entrypointType}"`);
    }

    const config = manifest.frameworkConfig;
    if (config) {
      if (!config.factory || typeof config.factory !== 'string') {
        errors.push('frameworkConfig.factory is required and must be a string');
      }
      ...
      if (config.invokeMode && !['invoke', 'stream', 'batch'].includes(config.invokeMode as string)) {
        errors.push(`frameworkConfig.invokeMode must be one of invoke, stream, batch`);
      }
    }
